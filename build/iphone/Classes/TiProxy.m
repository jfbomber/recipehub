...
//Rare exceptions to indicate a bug in the _recipehub code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_recipehub = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_recipehub]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.recipehub.module"
